...
int slaveShouldRemainInIOThread(client *slave) {
    serverAssert(slave->tid != IOTHREAD_MAIN_THREAD_ID);

    /* When processed by IO thread slave remains in main thread while waiting
     * for new replication data. We need to return the client to IO thread from
     * time to time so it can receive ACK from the slave node.
     * 500 is chosen empirically - standard replication cron runs every 1000ms
     * so 500 gives us a good balance to always catch the next ACK read in time. */
    const int return_for_ack_after_ms = 500;

    /* When client is using compression we make sure to flush its output every
     * compression_max_latency ms (see IOThreadCompressionCron). Just like for
     * ACK read we return it a bit more frequently to IO-thread so it doesn't
     * delay it much. */
    int return_for_compression_flush_after_ms = server.compression_max_latency / 2;

    return_for_compression_flush_after_ms =
        min(return_for_ack_after_ms, return_for_compression_flush_after_ms);

    mstime_t ms_since_last_ack = mstime() - slave->io_last_ack_time;

    int ms_to_return = slave->compression_state ?
        return_for_compression_flush_after_ms :
        return_for_ack_after_ms;

    return ms_since_last_ack >= ms_to_return;
}
...
void putSlavesNeedingAckReadInPendingClientsToIOThreads(void) {
    ...
    while((ln = listNext(&li))) {
        ...
        if (slave->tid != IOTHREAD_MAIN_THREAD_ID &&
            ...
            slaveShouldRemainInIOThread(slave))
        {
...
void replconfCommand(client *c) {
    ...
    for (j = 1; j < c->argc; j+=2) {
        if (!strcasecmp(c->argv[j]->ptr,"listening-port")) {
            ...
            serverLog(LL_NOTICE, "Client #%llu requests compression level %ld",
...
void readSyncBulkPayload(connection *conn) {
    ...
    /* If we agreed on compression with the master then setup conpression on the master client */
    /* At this point we're done reading all non compressed payload from the master */
    ...
    if (server.repl_state == REPL_STATE_RECEIVE_CLIENT_COMP) {
        ...
        if (err[0] == '-') {
            serverLog(LL_NOTICE,
                      ...
                      "REPLCONF compression, repl compression disabled: %s",
                      err);
        } else {
